            ...
            return (
              ...
              <div className={`absolute bottom-0 left-0 right-0 p-4 flex flex-col items-start transition-opacity duration-300 md:opacity-0 md:group-hover:opacity-100 ${isActive ? 'opacity-100' : 'opacity-0 md:opacity-0'}`}>
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    // Trigger Leadster chat
                    const w = window as Window & { neurolead?: { open?: () => void } }
                    if (w.neurolead?.open) {
                      w.neurolead.open()
                    } else {
                      // Fallback: click the Leadster widget button
                      const leadsterButton = document.querySelector('[data-leadster], .leadster-button, [class*="neurolead"]') as HTMLElement
                      leadsterButton?.click()
                    }
                  }}
